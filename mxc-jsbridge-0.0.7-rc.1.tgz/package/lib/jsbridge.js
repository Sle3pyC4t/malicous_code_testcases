...
exports.getCallbackMethodId = getCallbackMethodId;
...
function handleEventFromApp(data) {
  ...
  if (handlers && handlers.length) {
    ...
    (0, _setTimeout2["default"])(function () {
      (0, _forEach["default"])(handlers).call(handlers, function (fn) {
...
function removeEvent(eventName, handler) {
  ...
  if (!handler) {
    ...
    window.mexcAppJsBridge.events[eventName] = (0, _filter["default"])(events).call(events, function (fn) {
      return fn !== handler;
    });
  }
}
...
function callNative(method, params) {
  ...
  if (hasCallback) {
    promise = new _promise["default"](function (resolver) {
      callbackMethodName = getCallbackMethodId(resolver);
    });
  } else {
    promise = _promise["default"].resolve(null);
  }
  try {
    if (isAndroid) {
      window.mexcAppJsBridge.callNative(method, !params ? "" : (0, _stringify["default"])(params), callbackMethodName);
    } else {
      window.webkit.messageHandlers.mexcAppJsBridge.postMessage((0, _stringify["default"])({
        method: method,
        param: params,
 ...
 * @returns undefined
 */
function getCallbackMethodId(fn) {
